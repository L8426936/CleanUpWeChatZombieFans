...
(() => {
     ...
     */
    function initUI() {
        ui.friend_whitelist.setDataSource(db_util.findAllFriendWhitelistByLabel(label, 1));
        let total_page = db_util.getTotalPageByFriendLabelWhitelist(label);
    ...
    function modifyPageInfoShow(current_page) {
        if (Number.isInteger(current_page)) {
            ...
            ui.friend_whitelist.setDataSource(db_util.findAllFriendWhitelistByLabel(label, current_page));
    ...
    ui.clear_friends_button.on("click", () => {
        dialogs.build({
            ...
            db_util.deleteAllFriendWhitelistByLabel(label);
            db_util.deleteAllFriendLabelWhitelistByLabel(label);
            initUI();
        }).show();
    });
